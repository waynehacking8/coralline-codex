...
shell source. After the first client attaches, a global `client-attached` hook
enables tmux's native `destroy-unattached` session option, so the session is
destroyed only after its final client detaches. EXIT/HUP/INT/TERM traps retain
idempotent fallback cleanup for the watcher, private server, and temporary
directory.
...
active descendants it reads `session_meta`, `turn_context`, addressed
inter-agent task messages, collab lifecycle, task lifecycle, and `token_count`
records. It handles partial JSONL records,
detects session rollover, and atomically writes mode-0600 cache files.
Rendering performs no network request.

tmux supports up to five status rows. Coralline keeps one base row and expands
dynamically for one to four active agents. Nested overflow is shown explicitly,
and terminal agent states collapse immediately. Codex's `/agent` view remains
responsible for navigation and completed history.
...
| companion session context/tokens | active rollout `token_count` events | hidden until the first event |
| active subagents | rollout session/collab/task/token records | rows hidden when no agent is running |
...
- [Official Codex multi-agent protocol](https://github.com/openai/codex/blob/main/codex-rs/protocol/src/protocol.rs)
